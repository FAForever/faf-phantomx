...
function InitializeGameOptions()
	...
	if (tonumber(sessionInfo.Options.Mafia_Mod)) == 0 then
...
function SetPhantomEco(pEco)
	...
	if pData.isPhantom and tonumber(sessionInfo.Options.Mafia_Mod) == 0 then
...
function ShowHidePhantomElements(show) 
	if show then
		if pData.isPhantom then
			...
			if (tonumber(sessionInfo.Options.Mafia_Mod)) == 1 then
				pUI.box.meteorBtn:Hide()
			end	
